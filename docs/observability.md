...
by the app overlay. The dev observability overlay generates fixed-name
ConfigMaps from overlay-local files so Kustomize's default load restrictions
stay enabled:
...
The overlay-local Prometheus scrape configuration, Grafana datasource
provisioning, and Grafana dashboard provider provisioning are also generated by
Kustomize from files under `deploy/k8s/overlays/dev-observability`.
...
`deploy/k8s/overlays/dev-observability/grafana/dashboards/feature-flag-overview.json`.
Kustomize packages it into the `feature-flag-grafana-dashboard` ConfigMap, and
Grafana provisions it through the file dashboard provider mounted at
`/etc/grafana/dashboards`. Panels cover evaluation rate, enabled vs disabled
evaluation rate, evaluations by reason, update rate, kill switch events, scrape
availability, recent evaluation traffic, and basic JVM/process health signals.
