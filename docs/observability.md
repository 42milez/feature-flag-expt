...
by the app overlay. The script regenerates two fixed-name ConfigMaps from the
committed source artifacts on every run:

- `feature-flag-prometheus-rules` from
  `observability/prometheus/feature-flag.rules.yml`;
- `feature-flag-grafana-dashboard` from
  `observability/grafana/dashboards/feature-flag-overview.json`.

Those ConfigMaps are created by the script rather than Kustomize so Kustomize's
default load restrictions stay enabled while the committed rule and dashboard
files remain the single source of truth. The overlay-local Prometheus scrape
configuration, Grafana datasource provisioning, and Grafana dashboard provider
provisioning are still generated by Kustomize from files under
