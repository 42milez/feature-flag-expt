...
## :warning: フラグ評価方式の現在地とロードマップ

現時点では、機能フラグの設定・管理と、その設定に基づく実行時の評価を担うサービスです。機能フラグ利用側は、フラグキー・環境・テナント ID（必要に応じてユーザー ID）を `POST /api/evaluate` に渡し、返ってきた `enabled` をもとに旧機能と新機能のどちらを使うかを選ぶ想定です。

評価ロジックはこのサービス側にあり、利用側で評価を行う手段がまだないため、評価が必要になるたびに `POST /api/evaluate` を呼び出します。実装している範囲は、フラグ設定の永続化、環境ターゲティング、キルスイッチ、割合ロールアウト、高リスク変更の承認、監査イベントです。

この方式では評価のたびに通信が発生し、利用側は実行時にこのサービスへ依存します。そのため今後は、毎回問い合わせずに済む仕組み ― アプリ側でのキャッシュや、設定を配布して呼び出し側で評価する方式など ― の実装を検討しています。
...
| ワークフロー | トリガー | 対象 |
|---|---|---|
| `CI` | `main` への push、プルリクエスト、手動実行 | シークレットスキャン、フォーマット、Error Prone コンパイル、ユニットテスト、Testcontainers 統合テスト、JaCoCo カバレッジレポート生成、Kubernetes マニフェストのレンダリング検証、OpenAPI スナップショットの差分検出、Prometheus アラートルールの検証 |
...
| `Kind Smoke Test` | 毎日 18:00 UTC（03:00 JST）、手動実行 | kind クラスターでの起動検証。いずれかのステップが失敗した場合は Kubernetes の診断情報を収集 |

CI では `promtool` でアラートルールを検証します。イメージのワークフローは Docker イメージをローカルでビルドし、その同じイメージを Trivy でスキャンします。
...
HTTP Basic はローカルのポートフォリオ用の最低限の認証です。ユーザーの認証情報は意図的にアプリケーションのデータベースの外に置き、PostgreSQL はフラグの状態、ロールアウト設定、検証の挙動、監査イベントのために使います。ルートと権限の対応は `SecurityConfig` に直接記述し、余分な間接化なしにセキュリティ境界を読み取りやすく保っています。この API は curl などから呼び出すステートレスな JSON API として使う前提のため、CSRF トークン処理は無効化しています。ブラウザクライアントで使う場合のトレードオフや、本番では Basic を OIDC など組織で管理するアイデンティティプロバイダーへ置き換える方針は [ADR-0010](docs/decisions/0010-use-http-basic-for-local-portfolio-security-boundary.md) に記録しています。
...
オブザーバビリティスタックは、ローカルでアラート検証まで行える構成に意図的に限定しています（[ADR-0011](docs/decisions/0011-keep-observability-stack-alerting-ready-but-local.md)）。本番運用では、これに通知ルーティングや永続的なメトリクス保持、クラスター全体のログ収集などを別途追加することになります。
...
│       ├── policy/                          # ロールアウトポリシー: 検証ロジックは Java、API は Kotlin
│       ├── preview/                         # プレビュー: 評価ロジックは Java、API は Kotlin
│       ├── observability/                   # メトリクス計装（Java）
│       ├── error/                           # グローバルエラーハンドリング（Java）
│       └── SecurityConfig, ...
├── deploy/
│   ├── k8s/base/                            # アプリ（Deployment + Service）+ Namespace
│   ├── k8s/overlays/dev/                    # kind: クラスター内 PostgreSQL、ローカル設定
│   ├── k8s/overlays/dev-observability/      # Prometheus + Grafana + アラートルール
│   └── kind/cluster.yaml
├── compose.yaml                             # Docker Compose によるローカル実行環境（アプリ + PostgreSQL）
├── docs/
│   ├── decisions/                           # ADR
│   ├── development.md                       # ローカル実行・開発リファレンス（英語版）
│   ├── development.ja.md                    # ローカル実行・開発リファレンス（日本語版）
│   ├── observability.md                     # オブザーバビリティリファレンス
