...
現在の方式では、機能フラグ基盤がフラグの設定・管理を担い、その設定に基づいて実行時にフラグを評価します。機能フラグ利用側は、フラグキー・環境・テナント ID（必要に応じてユーザー ID）を `POST /api/evaluate` に渡し、返ってきた `enabled` をもとに旧機能と新機能のどちらを使うかを選ぶ想定です。評価ロジックは機能フラグ基盤側にあり、利用側で評価を行う手段がまだないため、評価が必要になるたびに `POST /api/evaluate` を呼び出します。

この方式では評価のたびに通信が発生し、利用側は実行時に機能フラグ基盤へ依存します。そのため今後は、毎回問い合わせずに済む仕組み ― アプリ側でのキャッシュや、設定を配布して呼び出し側で評価する方式など ― の実装を検討しています。
