...
| ワークフロー | トリガー | 対象 |
|---|---|---|
| `CI` | プルリクエストと手動実行 | フォーマット、Error Prone によるコンパイル、ユニットテスト、Testcontainers を使った統合テスト |
| `Kind Smoke Test` | 毎日 18:00 UTC（03:00 JST）と手動実行 | Spring Boot の jar のパッケージング、サービスの Docker イメージがビルドできること、kind クラスターでの Kubernetes マニフェストの起動検証 |
