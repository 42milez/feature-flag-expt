...
> **ステータス:** ポートフォリオ / 学習用プロジェクト。バックエンド開発の実践を端から端まで示すことを目的としており、本番サービスではありません。

フィーチャーフラグ管理サービスを題材に、**JVM でのドメイン設計、実際の Kubernetes デプロイ経路、オブザーバビリティ、CI 品質ゲート**を、1 つのレビューしやすいリポジトリで示すポートフォリオです。フラグは環境ごとのターゲティング、緊急停止用のキルスイッチ、テナント単位の許可リスト、フラグキーとテナント/ユーザー識別子から算出する決定的な割合バケットで制御でき、状態の変更はすべて監査イベントとして記録されます。
...
- **JVM サービス設計** — 永続化されるフラグのドメイン、評価ロジック、Spring Data JDBC のトランザクションフロー、監査イベントの記録、Micrometer の計装、Spring Security の境界は Java が担当します。リクエスト/レスポンスの組み立てが中心になるプレビューとロールアウトポリシーの API は Kotlin が担当し、再利用されるポリシーバリデーターは Java のまま本番の更新経路と共有します。([ADR-0008](docs/decisions/0008-use-kotlin-for-evaluation-preview-api.md))
- **フェイルクローズなセキュリティ境界** — reader/operator ロールを持つローカルの HTTP Basic 認証。公開するのはプローブと API ドキュメントだけで、分類されていない `/api/**` ルートは既定で拒否します。([ADR-0010](docs/decisions/0010-use-http-basic-for-local-portfolio-security-boundary.md))
- **実際の Kubernetes 経路** — Kustomize の `base` と `dev` オーバーレイを kind にデプロイし、Pod Security Standards の [restricted](https://kubernetes.io/docs/concepts/security/pod-security-standards/#restricted) プロファイル（非 root、読み取り専用ルートファイルシステム、capabilities 全削除、RuntimeDefault seccomp、グレースフルシャットダウン）に沿ってハードニングします。([ADR-0009](docs/decisions/0009-use-kind-for-local-kubernetes-development-and-ci-validation.md))
- **証拠としてのオブザーバビリティ** — Actuator/Micrometer のメトリクス、ECS JSON の構造化ログ、`promtool` のルールテスト付きでコミットした Prometheus アラートルール、Grafana ダッシュボード。([ADR-0011](docs/decisions/0011-keep-observability-stack-alerting-ready-but-local.md))
...
フラグのドメイン、評価ロジック、永続化、監査の振る舞いは Java で実装しています。プレビューとロールアウトポリシーの API は、変更案・サンプルごとの差分・検証結果を表現するために Kotlin で実装し、再利用されるロールアウトポリシーバリデーターは Java のまま本番の更新経路と共有しています。
...
> `bucket` は `floorMod(SHA-256(flagKey + ":" + identity), 100)` です。同じフラグキーと識別子は常に同じバケットに入るため、リクエストごとにランダムになるのではなく、安定した決定的なロールアウトになります。
...
コンテナはデータベースプロセスのみを起動します。スキーママイグレーションはアプリケーション起動時に Flyway が適用します。既定値の変更は [デプロイと運用](#デプロイと運用) を参照してください。
...
Trivy ゲートは、**修正済み**の High / Critical の OS・ライブラリ脆弱性で失敗し、未修正の検出結果は失敗条件から除外します。また、未修正の High / Critical を*含む*非ブロッキングのジョブサマリーを出力するため、ゲートを失敗させないリスクもレビュー時に確認できます。スケジュール実行は、アプリケーションコードに変更がなくても新しい CVE の公開で赤くなることがあります。これは脆弱性ゲートとして想定される挙動であり、未修正の検出結果を除外してもその可能性は小さくなるだけで完全にはなくなりません。

</details>

### 設定

接続可能な PostgreSQL インスタンスが必要です。以下の既定値はクイックスタートのコンテナと一致するため、ローカル実行に追加設定は不要です。別のデータベースを指したい場合や認証情報を変えたい場合は、これらの環境変数を上書きします。
...
マイグレーションは `service/src/main/resources/db/migration` 以下にあり、アプリケーション起動時に Flyway が適用します。コンテナの初期化フローではなく、この起動時適用を標準のマイグレーション経路としています。

### セキュリティ

API アクセスにはローカルの HTTP Basic ユーザーを 2 つ使います。参照系の **reader** と、作成・更新系の **operator** です。Prometheus メトリクスには任意の設定済みユーザーが必要で、Swagger UI と OpenAPI ドキュメントはローカルで確認しやすいよう公開のままにしています。監査イベントは認証済みプリンシパルを `actor` として記録します。actor はサービスが Spring Security から導出するもので、リクエストのペイロードからは受け取りません。

<details>
<summary>セキュリティモデルの範囲と発展</summary>

HTTP Basic はローカルのポートフォリオ用の最低限の認証です。ユーザーの認証情報は意図的にアプリケーションのデータベースの外に置き、PostgreSQL はフラグの状態、ロールアウト設定、検証の挙動、監査イベントのために使います。ステートレスな JSON API のため CSRF は無効化していますが、HTTP Basic はブラウザでは CSRF の影響を受けるため、本番でブラウザからアクセスする場合は CSRF 保護を再有効化するか Basic を置き換える必要があります。起動時のパスワードエンコードはインメモリのユーザーストア用であり、環境変数や Kubernetes Secret そのものを保護するものではありません。

ルートと権限の対応は `SecurityConfig` に直接記述し、余分な間接化なしに境界を読み取りやすく保っています。本番システムでは、`flags:read`・`flags:write`・`metrics:read` のような操作単位の権限、メソッドセキュリティ、外部認可レイヤーへ発展させたり、HTTP Basic を OIDC など組織で管理するアイデンティティプロバイダーに置き換えて同等の reader/operator 権限にマッピングしたりできます。[ADR-0010](docs/decisions/0010-use-http-basic-for-local-portfolio-security-boundary.md) を参照してください。
