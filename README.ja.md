...
Docker Compose は、シンプルなローカルアプリケーション実行環境としてのみ提供しています。Kubernetes マニフェスト、サービスディスカバリ、プローブ、`kubectl apply` の検証経路は引き続き kind です。Compose はアプリを `127.0.0.1:8080` にバインドするため `k8sPortForward` と競合します。Compose と kind のポートフォワード経路は分けて使ってください。データベースを必要とする統合テストは引き続き Testcontainers で実行します。詳細は [ADR-0009](docs/decisions/0009-use-kind-for-local-kubernetes-development-and-ci-validation.md) を参照してください。
