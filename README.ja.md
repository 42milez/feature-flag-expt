...
- **CI 品質ゲート** — フォーマット、Error Prone、全テストと Testcontainers テスト、JaCoCo カバレッジの Codacy へのアップロード、Kubernetes マニフェストのレンダリング検証、OpenAPI スナップショットの差分検出、`promtool` チェック、Trivy のシークレット/イメージスキャンを、変更ごとに実行します。
...
実行したい経路に合わせて、必要最小限のツールだけを用意してください。

### Docker だけのクイックスタート

`docker compose` コマンドを使える Docker 環境が必要です。macOS では [Docker Desktop](https://docs.docker.com/desktop/) や [OrbStack](https://docs.orbstack.dev/install)、Linux では Docker Engine と Compose plugin など、利用できる環境に合わせて用意してください。

### kind / Kubernetes ローカル検証

`docker` に加えて、`kind` と `kubectl` が必要です。kind へのデプロイとローカルの Prometheus/Grafana 検証コマンドは、これらのツールに依存します。公式手順は [kind のインストール](https://kind.sigs.k8s.io/docs/user/quick-start/#installation) と [Kubernetes tools](https://kubernetes.io/docs/tasks/tools/) を参照してください。

### ホスト JVM 開発

JDK 25 が必要です。CI と揃えるため [Eclipse Temurin 25](https://adoptium.net/temurin/releases/?version=25) を推奨します。この経路はホスト側でのテスト、`bootRun`、OpenAPI 生成、Gradle 補助タスクに必要です。このプロジェクトは macOS、Linux、または Windows では WSL 経由での実行を想定しています。一部の Gradle タスクがシェルスクリプトや Unix 系ツールを呼び出すためです。

## クイックスタート

3 ステップでフラグを作成・評価します。[Docker だけのクイックスタートの事前準備](#docker-だけのクイックスタート)を使う経路で、ホスト側に JDK をインストールする必要はありません。
...
サービスを起動するには PostgreSQL が必要です。Compose クイックスタートでは、アプリコンテナが `postgres` サービスへ自動的に接続します。ホストから JVM を直接起動する場合は、下記の既定値で `localhost:5432` の PostgreSQL に接続できます。別のデータベースを使う場合やユーザー名・パスワードを変える場合は、対応する環境変数を上書きしてください。
...
[kind / Kubernetes ローカル検証の事前準備](#kind--kubernetes-ローカル検証)を済ませてから、kind ワークフローを実行してください。
...
`dev` オーバーレイは `base` の上に、クラスター内 PostgreSQL、ローカルのデータベース設定、プレースホルダーの認証情報、`kind load` で使うローカルのイメージタグを追加します。

Docker Compose は、シンプルなローカルアプリケーション実行環境としてのみ提供しています。Kubernetes マニフェスト、サービスディスカバリ、プローブ、`kubectl apply` の検証経路は kind を使用します。なお、Compose はアプリを `127.0.0.1:8080` にバインドするため `k8sPortForward` と競合します。Compose と kind のアプリ公開経路は同時に使わず、どちらか一方を選んでください。データベースを必要とする統合テストは引き続き Testcontainers で実行します。詳細は [ADR-0009](docs/decisions/0009-use-kind-for-local-kubernetes-development-and-ci-validation.md) を参照してください。
...
[ホスト JVM 開発の事前準備](#ホスト-jvm-開発)を済ませてから、JVM で直接開発してください。そのホスト側ツールチェーンをインストール済みの開発者は、ローカル Compose データベースだけを起動し、ホストから `bootRun` でサービスを起動します。
...
```bash
./gradlew composeConfig # Compose 設定を検証
./gradlew composeUp     # Gradle 経由で Compose スタックを起動
./gradlew composeDown   # Compose スタックを停止・削除
```

### VS Code でデバッグする

共有の VS Code 設定は `.vscode/launch.json`、`.vscode/tasks.json`、`.vscode/extensions.json` に置いています。ワークスペースの推奨拡張として表示される Java Extension Pack、Gradle for Java、Kotlin をインストールしてください。ローカル JDK のパスのようなマシン固有の設定は、コミット対象外の個人用 `.vscode/settings.json` に置きます。

このワークスペースには、アプリケーションへ attach するデバッグ構成が 2 つあります。どちらもローカルポート `5005` を使うため、一方のデバッグモードを止めてからもう一方を起動してください。

**モード 1: ホスト JVM のアプリ + Docker PostgreSQL**

```bash
docker compose up -d postgres
```

その後、VS Code で **Attach to local service bootRun** を実行します。pre-launch task が `${workspaceFolder}/gradlew :service:bootRun --debug-jvm` を起動し、`localhost:5005` で待ち受けます。アプリは既定のローカル JDBC URL `jdbc:postgresql://localhost:5432/featureflags` で PostgreSQL に接続します。

**モード 2: kind のアプリ Pod + kind PostgreSQL**

```bash
./gradlew debugDeploy
```

その後、VS Code で **Attach to kind service** を実行します。`debugDeploy` はローカルイメージのビルドと kind へのロード、`debug` オーバーレイの適用、PostgreSQL とアプリの待機、Pod ステータス表示まで行います。VS Code の pre-launch task は debug 用の port-forward を開始するだけです。

```bash
kubectl -n feature-flag-platform port-forward deployment/feature-flag-platform 5005:5005
```

この task は Pod のデプロイやパッチ適用は行いません。

kind のデバッグ経路では、アプリと PostgreSQL の両方が kind クラスター内で動きます。`debugDeploy` を省略すると、アプリ Pod が存在せず port-forward に失敗することがあります。また、通常の `dev` Pod へ port-forward できても JDWP が有効ではないため、VS Code の attach は拒否されます。debug オーバーレイは `suspend=n` を使うので、VS Code が未接続でも Kubernetes の startup/readiness probe はブロックされません。

debug オーバーレイは Dockerfile の `JAVA_TOOL_OPTIONS=-XX:MaxRAMPercentage=75.0` を意図的にコピーし、そこへ JDWP を追加しています。Kubernetes のコンテナ環境変数はイメージ側の `ENV` を上書きするため、今後 Dockerfile の `JAVA_TOOL_OPTIONS` を変更した場合は `deploy/k8s/overlays/debug/patches/enable-jdwp.yaml` も同期してください。
...
./gradlew :service:jacocoTestReport # JaCoCo の XML / HTML カバレッジレポートを生成
...
│   ├── k8s/overlays/debug/             # kind: dev オーバーレイ + JDWP attach ポート
