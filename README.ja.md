...
| ワークフロー | トリガー | 対象 |
|---|---|---|
| `CI` | `main` への push、プルリクエスト、手動実行 | フォーマット、Error Prone コンパイル、ユニットテスト、Testcontainers 統合テスト、JaCoCo カバレッジレポート生成とガード付き Codacy アップロード、Kubernetes マニフェストのレンダリング検証、OpenAPI スナップショットの差分検出、Prometheus アラートルールの検証 |
...
Codacy はカバレッジの可視化、保守性の傾向、重複、複雑度、コードスメルのフィードバックを担当します。フォーマットの基準は引き続き Spotless、コンパイル時の Java 静的解析ゲートは Error Prone、リポジトリのシークレットスキャンとビルド済みイメージの脆弱性スキャンは Trivy が担当します。
