...
プロダクトチームが安全に変更をリリースするための社内開発者向け基盤を想定し、Spring Boot 製フィーチャーフラグサービスを題材にしたポートフォリオです。環境ごとのターゲティング、緊急停止用のキルスイッチ、テナント単位の許可リスト、決定的な割合ロールアウト、監査イベントを扱います。アプリケーション、デプロイ用マニフェスト、オブザーバビリティ、CI 品質ゲートを 1 つのリポジトリにまとめ、デリバリー経路を端から端までレビューできる構成にしています。
