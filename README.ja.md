...
- **実際の Kubernetes 経路** — Kustomize の `base` と `dev` オーバーレイを kind にデプロイし、Pod Security Standards の *restricted* プロファイル（非 root、読み取り専用ルートファイルシステム、capabilities 全削除、RuntimeDefault seccomp、グレースフルシャットダウン）に沿ってハードニングします。([ADR-0009](docs/decisions/0009-use-kind-for-local-kubernetes-development-and-ci-validation.md))
