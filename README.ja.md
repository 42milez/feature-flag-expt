...
プロダクトチームが安全に変更をリリースするための、Spring Boot 製フィーチャーフラグ基盤サービスです。環境ごとのターゲティング、緊急停止用のキルスイッチ、テナント単位の許可リスト、決定的な割合ロールアウト、監査イベントを備えています。アプリケーション、デプロイ用マニフェスト、オブザーバビリティ、CI 品質ゲートを 1 つのリポジトリにまとめ、デリバリー経路を端から端までレビューできる構成にしています。
