...
class FeatureFlagRepositoryIntegrationTest extends PostgreSqlIntegrationTest {
  ...
  void savesAndLoadsFlagWithChildCollections() {
    FeatureFlagEntity saved =
        repository.save(
            FeatureFlagEntity.create(
                "checkout-redesign",
                FeatureFlagStatus.ENABLED,
                Set.of(
                    new TargetEnvironmentEntity("production"),
                    new TargetEnvironmentEntity("staging")),
                false,
                Set.of(
                    new TenantAllowlistEntity("tenant-a"), new TenantAllowlistEntity("tenant-b")),
                25));
  ...
  void savingExistingFlagReplacesChildCollections() {
    repository.save(
        FeatureFlagEntity.create(
            ...
            FeatureFlagStatus.ENABLED,
            Set.of(
                new TargetEnvironmentEntity("production"), new TargetEnvironmentEntity("staging")),
            false,
